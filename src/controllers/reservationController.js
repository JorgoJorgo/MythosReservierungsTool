...
router.post('/', auth, async (req, res) => {
  ...
  console.log('[POST /api/reservations] User ID from JWT:', req.user.userId);
...
router.put('/:id', auth, async (req, res) => {
  ...
  try {
    ...
    const updatedReservation = await pool.query(
      'UPDATE reservations SET date = $1, time = $2, customer_name = $3, num_guests = $4, table_name = $5, phone_number = $6 WHERE id = $7 RETURNING *',
      [date, time, customerName, numGuests, tableName, phoneNumber, id]
    );
